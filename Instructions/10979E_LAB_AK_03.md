...
1. In the Microsoft Edge window, in the Azure Portal, ensure that you are viewing the **10979E0301-vm** blade. 
...
1. Switch to the Remote Desktop session window. If prompted, in the Networks pane, click **No**.
...
1. In the Remote Desktop session window, switch to the **Server Manager** window.

1. In the Server Manager window, click **Local Server**.

1. In PROPERTIES For 10979E0301-vm, beside **IE Enhanced Security Configuration**, click **On**.

1. In the Internet Explorer Enhanced Security Configuration window, below **Administrators:**, select **Off**, and click **OK**.

1. Start Internet Explorer.

1. Select **Use recommended security, privacy, and compatibilitiy settings**, and click **OK**.

1. Browse to [https://docs.microsoft.com/en-us/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-2017]([https://docs.microsoft.com/en-us/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-2017]).

1. In the **SSMS 17.9 is the current General Availability (GA) version of SSMS** section, click **Download SQL Server Management Studio 17.9**.

1. In the message **Do you want to run or save SSMS-Setup-Enu.exe (807 MB) from download.microsoft.com?**, click the down-arrow beside the **Save** button, and click **Save as**.

1. Save the file **SSMS-Setup-ENU.exe** to D:\. Wait for the download to finish.
...
1. Browse to D:\ and double-click **DATALOSS_WARNING_README.txt**.

1. Review the contents of DATALOSS_WARINING_README.txt and close the Notepad window.

1. In File Explorer, double-click **SSMS-Setup-ENU.exe**. In the Security Warning window, click **Run**.

1. In the Microsoft SQL Server Management Studio 17.9 (32 bit) setup program, click **Install**. Wait for the setup to finish.

1. When Microsoft SQL Server Management Studio Setup has completed, click **Close**.
