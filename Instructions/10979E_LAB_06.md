...
For this lab, you need Azure VM you created in Lab: Creating a VM in Azure of Module 3.
...
1. In the Microsoft Edge window, in the Azure Portal, establish an RDP connection to the **10979E0301-vm** VM by using the following credentials:
...
1. In the Remote Desktop session windows, start Internet Explorer and browse to [**http://portal.azure.com**](http://portal.azure.com). Sign in to the Azure Portal as the Service Administrator of your Azure subscription.
...
1. In the Remote Desktop session window, start SQL Server Management Studio.
...
1. If the connection attempt fails indicating that the client IP address is not allowed to access the server, note the IP address in the error message. Switch to Internet Explorer and, on the **Firewall settings** blade, create a new rule by specifying the following settings:
...
1. From the Azure Portal, delete the **10979E0301-LabRG**, **10979E0601-LabRG**, and **10979E0602-LabRG** resource groups.
